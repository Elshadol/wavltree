 ...
 * 1) Every rank difference is 1 or 2.
 * 2) Every leaf has rank 0.
 *
 * In this implementation, we use rank parity instead of rank.
 * If a node and its parent have the same rank parity, it means rank difference 
 * of the node is 0 or 2.
 * If a node and its parent don't have the same rank parity, it means rank 
 * difference of the node is 1 or 3.
 */
...
{
    /* The rank of the null node is -1, so its rank parity is 1 */
...
void wavl_insert_fixup(struct wavl_node* node, struct wavl_root* root)
{
    ...
    while (1) {
        /* Loop invariant: the rank of the tree rooted at node grew */
        ...
        /* node now become an 1-child, we're done. */
        if (p1 != __wavl_parity(parent))
            break;

        /*
         * Since node and parent have the same rank parity,
         * we flip p1 for promoting/demoting node or parent later.
         */
        p1 ^= 1lu;
        ...
        if (node != tmp1) { /* node == parent->wavl_left */
            if (p1 == wavl_parity(tmp1)) {
                /*
                 * Case 1: node’s sibling is an 1-child.
                 ...
                 * doesn't allow this, we need recurse at parent.
                 */
                tmp2 = node;
                node = parent;
            ...
            if (tmp1 == tmp2) {
                 ...
                 * is an 1-child (left rotate at node).
                 *
                 *      n-p              t1-p
                 *     / \ \     -->     /   \
                 *    /  t1 \           n     \
             ...
             * is an 1-child (right rotate at parent).
             *
             *      n-p    -->   n
             *     / \ \        / \
             *    t2  \ \     t2   p
                                      ...
                                      struct wavl_root* root)
{
    ...
    if (parent->wavl_right == parent->wavl_left) {
    ...
    do {
        /* Loop invariant: the rank of the tree rooted at node shrank */

        p2 = __wavl_parity(parent);

        /* node now is a 2-child, we're done. */
        if (p1 == p2)
            break;
        ...
        if (node != sibling) { /* node == parent->wavl_left */
            ...
            if (p2 != wavl_parity(tmp1)) {
                ...
                /*
                 * Case 3: node's sibling is an 1-child and node's far
...
void wavl_erase(struct wavl_node* node, struct wavl_root* root)
{
    ...
    if (child) {
        p1 = 0lu;
        wavl_set_parent_parity(child, parent, p1);
    }
    if (parent)
